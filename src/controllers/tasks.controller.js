...
exports.createTask = function(req, res) {
  con.query(queries.INSERT_TASK, [req.body.name, req.body.status, req.body.entry], function(err, result) {
    if (err) {
      res.send(err);
    }
    console.log(result);
...
exports.updateTask = function(req, res) {
  con.query(
    queries.UPDATE_TASK,
    [req.body.name, req.body.status, req.body.entry, req.params.taskId],
    function(err, data) {
      if (err) {
        res.send(err);
      }
